...
class MoveItEEClient(Node):
    ...
    def _goal_response_cb(self, future):
        ...
        if not goal_handle.accepted:
            self.get_logger().error("MoveIt goal rejected") # error message when the robot overreaches or the movement is too computationally expensive
            return
        self.get_logger().info("MoveIt goal accepted")
        goal_handle.get_result_async().add_done_callback(self._result_cb) # call at the very end
    ...
    def _result_cb(self, future):
        ...
        self.get_logger().info(f"[Result] error_code {code}") # error message
